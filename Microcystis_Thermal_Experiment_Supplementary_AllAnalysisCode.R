...
#######################################################################################
# Figure 1 ----------------------------------------------------------------
tree<-read.tree("RAxML_bipartitions.tree.txt")
...
full_tree_p<-ggtree(tree) + geom_tree() +  theme_tree() +  geom_tiplab(size=7,hjust = -0.25)
...
data_exp <- data_exp %>% mutate(Growth_transformed = ((Growth + 1)^lambda - 1)/lambda) #normality of data is improved so transform data
...
data_exp_remO <-data_exp[(data_exp$Outlier=="n"),]# Removing an LN/LG outlier from the main dataset for plotting only, but it is added back in below 
...
#see also the figure at https://dfzljdn9uc3pi.cloudfront.net/2019/6876/1/fig-4-2x.jpg from the same article for explanation of models
...
#normality of data is improved so transform data
data <- data %>%
  mutate(Growth_transformed = ((Growth + 1)^lambda - 1)/lambda) 

data$Phase <- factor(data$Phase, levels = c("Exp", "wk2", "wk3", "wk4")) #make sure Phase is a factor for in models
...
anova(lim_ints,two_way) #lim_ints wins

#switch back to REML method for final version now model comparison is completed
lim_ints <- lme(Growth_transformed~ factor(combo) + factor(temp) + factor(Phase)+ 
                  factor(combo):factor(temp) + factor(combo):factor(Phase), 
...
GS <-  gam(Growth ~ s(temp, k=4, m=2) + s(strain, k = 19, bs = 're') +
           ...
           method = 'REML',
           data = data)
#ignore warning, this is because of having both global and group level trends across temp for growth rate
...
#these are useful for ordered categorical data
...
tiff("./Final figure files/Fig3.tiff", units="px", width=2100, height=1480, res=200)
Fig_3
dev.off()
...
sdat <- dat %>%
  ...
  #group by factors identifying unique treatments, also include any other factors that need to be kept in the output
...
primer <- unique(sdat$Target) #create a vector of gene targets to help with plotting
...
hsps <- tdat %>% dplyr::filter(!Target %in% c("Hep")) #remove Hep from the general model
...
primer <- unique(tdat$Target) #make a vector of the target genes to loop through
...
#######################################################################################
# Figure 5 ------------------------------------------------------------------
tdat <- read.csv("tdat.csv") #expression data
data<-read.csv("Growth_data.csv") #growth data
data<-data[(data$Keep=="y"),]# This removes CR19-01, which is the sole LN/HG
...
vegan::permutest(vegan::betadisper(t_mat_dist, tdat_wide$combo)) #dispersion is okay so can go ahead with lda
...
#######################################################################################
# Figure S1 ---------------------------------------------------------------
full_tree<-read.tree("RAxML_bipartitions.FigS1A.txt")
...
### Fig S1B
full_its_tree<-read.tree("RAxML_bipartitions.FigS1B.txt")
...
data <- data %>%
  mutate(Growth_transformed = ((Growth + 1)^lambda - 1)/lambda) %>% 
  #also remove unnecessary columns of the data
  dplyr::select(flask, strain, Lake, combo_temp, combo, genotype, Trophic, temp, Crash_Numeric, Crash, Growth_transformed, Phase) %>% 
  #and widen data so each Phase has its own growth data column
  dcast(flask + strain + temp + genotype + Trophic + combo + Crash_Numeric ~ Phase, 
        value.var = "Growth_transformed") %>% 
